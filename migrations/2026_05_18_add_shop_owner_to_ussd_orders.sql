-- Add shop_owner_id to ussd_orders so registered users' USSD purchases
-- appear on their My Orders page via combined_orders_view.
ALTER TABLE ussd_orders
  ADD COLUMN IF NOT EXISTS shop_owner_id UUID REFERENCES users(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_ussd_orders_shop_owner ON ussd_orders(shop_owner_id)
  WHERE shop_owner_id IS NOT NULL;

-- Rebuild combined_orders_view to propagate shop_owner_id from ussd_orders.
-- (Previously hardcoded NULL for USSD rows.)
