 ...
 *
 * Polling fallback for SPFastIT orders whose webhook was missed. Mirrors
 * the Xpress cron's structure (sequential polling, no batch/retry
 * disambiguation needed — SPFastIT gives a stable transaction_id per order
 * with documented idempotent-recovery on resubmission, with no documented
 * retry-splits-into-new-order behavior like AgentPortalGH's).
 */
