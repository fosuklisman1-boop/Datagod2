...
export async function GET(request: NextRequest) {
    ...
    try {
        console.log("[CRON-CODECRAFT] Starting status sync...")

        const { data: pendingOrders, error: fetchError } = await supabase
            .from("mtn_fulfillment_tracking")
            .select("id, mtn_order_id, status, shop_order_id, order_id, api_order_id, order_type")
            .eq("provider", "codecraft")
            .in("status", ["pending", "processing"])
            .order("created_at", { ascending: true })
            .limit(BATCH_SIZE)

        if (fetchError) {
            console.error("[CRON-CODECRAFT] Error fetching orders:", fetchError)
            return NextResponse.json({ error: "Failed to fetch orders" }, { status: 500 })
        }

        if (!pendingOrders || pendingOrders.length === 0) {
            return NextResponse.json({ success: true, message: "No CodeCraft MTN orders to sync" })
        }

        console.log(`[CRON-CODECRAFT] Found ${pendingOrders.length} orders to sync`)
        ...
        for (let i = 0; i < pendingOrders.length; i++) {
            ...
            try {
                console.log(`[CRON-CODECRAFT] Syncing ${i + 1}/${pendingOrders.length}: ${order.mtn_order_id}`)

                const result = await checkMTNOrderStatus(order.mtn_order_id, "codecraft")

                if (!result.success && result.message?.includes("429")) {
                    console.warn(`[CRON-CODECRAFT] Rate limited on ${order.mtn_order_id}, waiting ${DELAY_ON_429_MS}ms...`)
                    rateLimited++
                    failed++
                ...
                if (result.success && result.status) {
                    ...
                    if (newPriority < currentPriority) {
                        console.log(`[CRON-CODECRAFT] ⛔ Skipping regression ${oldStatus} -> ${newStatus} for ${order.mtn_order_id}`)
                    } else if (newStatus !== oldStatus) {
                        await supabase
                            .from("mtn_fulfillment_tracking")
                            .update({
                                status: newStatus,
                                external_status: result.order?.data?.order_status || newStatus,
                        ...
                        if (order.order_type === "bulk" && order.order_id) {
                            const { data: orderData, error: orderError } = await supabase
                                ...
                                .single()
                            if (orderError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to update bulk order ${order.order_id}:`, orderError)
                            } else if (orderData) {
                            ...
                            const { data: apiData, error: apiError } = await supabase
                                ...
                                .single()
                            if (apiError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to update API order ${apiId}:`, apiError)
                            } else if (apiData) {
                            ...
                            const { data: ussdData, error: ussdError } = await supabase
                                ...
                                .single()
                            if (ussdError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to update USSD order ${order.order_id}:`, ussdError)
                            } else if (ussdData) {
                            ...
                            const { data: ussdShopData, error: ussdShopError } = await supabase
                                ...
                                .single()
                            if (ussdShopError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to update USSD shop order ${order.order_id}:`, ussdShopError)
                            ...
                            const { data: shopData, error: shopError } = await supabase
                                ...
                                .single()
                            if (shopError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to update shop order ${order.shop_order_id}:`, shopError)
                            } else if (shopData) {
                        ...
                        if (userId && (newStatus === "completed" || newStatus === "failed")) {
                            const notifTitle = newStatus === "completed" ? "Order Delivered Successfully" : "Order Delivery Failed"
                            ...
                            const actionUrl = order.order_type === "bulk"
                            ...
                            if (notifError) {
                                console.error(`[CRON-CODECRAFT] ⚠️ Failed to send notification for ${order.mtn_order_id}:`, notifError)
                            } else {
                                console.log(`[CRON-CODECRAFT] 🔔 Notification sent to user ${userId} for ${order.mtn_order_id} (${newStatus})`)
                            }
                        ...
                        console.log(`[CRON-CODECRAFT] ✅ ${order.mtn_order_id}: ${oldStatus} -> ${newStatus}`)
                        synced++
                    } else {
                        console.log(`[CRON-CODECRAFT] ${order.mtn_order_id}: unchanged (${newStatus})`)
                    }
                } else {
                    console.warn(`[CRON-CODECRAFT] Failed to get status for ${order.mtn_order_id}:`, result.message)
                    failed++
                }
                ...
                console.error(`[CRON-CODECRAFT] Error processing ${order.mtn_order_id}:`, err)
                failed++
            }
        }
        ...
        let reversed = 0
        const reversalCandidates = await fetchReversalCandidates(supabase, "codecraft", BATCH_SIZE)
        for (const cand of reversalCandidates) {
            const chk = await checkMTNOrderStatus((cand as any).mtn_order_id, "codecraft")
        ...
        console.error("[CRON-CODECRAFT] Critical error:", error)
