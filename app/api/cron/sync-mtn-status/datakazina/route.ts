...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        for (let i = 0; i < pendingOrders.length; i++) {
            ...
            try {
                ...
                if (result.success && result.status) {
                    ...
                    // Prevent status regression: DataKazina may report "pending" while
                    // the order is queued on their side — never move backwards.
                    ...
                    if (newPriority < currentPriority) {
                        console.log(`[CRON-DATAKAZINA] ⛔ Skipping regression ${oldStatus} -> ${newStatus} for order ${order.mtn_order_id}`)
        ...
        let reversed = 0
        const reversalCandidates = await fetchReversalCandidates(supabase, "datakazina", BATCH_SIZE)
        for (const cand of reversalCandidates) {
            const chk = await checkMTNOrderStatus((cand as any).mtn_order_id, "datakazina")
