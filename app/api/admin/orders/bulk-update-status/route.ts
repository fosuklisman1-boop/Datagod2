...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (status === "completed") {
      ...
      skippedPendingCount = (bulkOrders?.length || 0) + (shopOrders?.length || 0) + (apiOrders?.length || 0) + (ussdOrders?.length || 0) + (ussdShopOrders?.length || 0)
        - (allowedBulk.length + allowedShop.length + allowedApi.length + allowedUssd.length + allowedUssdShop.length)
