...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[AgentPortalGH Admin] GET action=${action} error:`, error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "AgentPortalGH request failed" },
      { status: 502 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[AgentPortalGH Admin] POST action=${action} error:`, error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "AgentPortalGH request failed" },
      { status: 502 }
    )
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[AgentPortalGH Admin] PUT action=${action} error:`, error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "AgentPortalGH request failed" },
      { status: 502 }
    )
