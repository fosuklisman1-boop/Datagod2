  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!phoneCheckData.available) {
        toast.error(phoneCheckData.error || "This phone number is already registered")
        return
      }

      // Create profile via existing signup route
      ...
      toast.success("Profile created! Welcome to DATAGOD.")
        ...
        <Card className="w-full max-w-md shadow-sm">
          <CardContent className="text-center space-y-4 py-10">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-destructive/10 mb-2">
              <svg className="w-8 h-8 text-destructive" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
            </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md shadow-sm">
          <CardContent className="p-6 sm:p-8">
            ...
            {/* Avatar + greeting */}
            <div className="mx-auto mt-4 grid h-16 w-16 place-items-center rounded-full bg-gradient-to-br from-primary to-primary text-2xl font-extrabold text-primary-foreground">
